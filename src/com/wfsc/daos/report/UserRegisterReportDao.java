...
public class UserRegisterReportDao extends EnhancedHibernateDaoSupport<UserRegisterReport> {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<Integer, Integer> getYearData(){
		return getHibernateTemplate().execute(new HibernateCallback<Map<Integer, Integer>>(){
			...
			public Map<Integer, Integer> doInHibernate(Session session) throws HibernateException, SQLException {
				String hql = "select year, sum(regCount) from UserRegisterReport group by year order by year asc";
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<Integer, Integer> getMonthData(final int year){
		return getHibernateTemplate().execute(new HibernateCallback<Map<Integer, Integer>>(){
			@Override
			public Map<Integer, Integer> doInHibernate(Session session) throws HibernateException, SQLException {
				String hql = "select month, sum(regCount) from UserRegisterReport where year = ? group by month order by month asc";
				Query query = session.createQuery(hql);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<Integer, Integer> getWeekData(final int year){
		return getHibernateTemplate().execute(new HibernateCallback<Map<Integer, Integer>>(){

			@Override
			public Map<Integer, Integer> doInHibernate(Session session) throws HibernateException, SQLException {
				String hql = "select week, regCount from UserRegisterReport where year = ? order by week asc";
				Query query = session.createQuery(hql);
				query.setInteger(0, year);
				List<Object[]> list = query.list();
				if(CollectionUtils.isNotEmpty(list)){
