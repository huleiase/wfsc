...
public class AdminDao extends EnhancedHibernateDaoSupport<Admin> {
	...
	public Admin getUserWithPermissionByName(String userName) {
		...
		if (CollectionUtils.isNotEmpty(result)) {
			...
			Hibernate.initialize(userObj.getGroups());
...
//		getHibernateTemplate().saveOrUpdate(admin);
	...
	public Page<Admin> findPageForAdmin(Page<Admin> page, Map<String,Object> paramap){
		...
		for(String key : paramap.keySet()){
			...
			if("area".equals(key)){
				hql.append(" and a.area ='"+paramap.get(key)+"'");
				continue;
			}
		...
		int totalCount = 0;
		List<Admin> listCount = this.findList4Page(hql.toString(), 0, Integer.MAX_VALUE);
		if(listCount!=null){
		...
		for(Admin a : list){
			...
			if(CollectionUtils.isNotEmpty(a.getGroups())){
				String groupString = "";
				for(UserGroup r : a.getGroups()){
					groupString += r.getGroupName()+",";
				}
				a.setRoleString(groupString);
			}
		}
		page.setData(list);
		return page;
