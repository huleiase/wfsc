...
<script type="text/javascript">
	...
	function addOrUpdate(id){
		var url = "<%=basePath%>admin/supplier_input.Q";
		if(id){
			url+="?id="+id;
		}
	...
	function deleteByIds(){
		...
		if(selectCheckbox.length<1){
			...
			art.dialog({
			    ...
			    ok: function () {
			    	$.ajax({
						url:"<%=basePath%>admin/supplier_deleteByIds.Q",
						data:{"ids":ids},
						dataType:'text',
	...
	function disableByIds(){
		...
		if(selectCheckbox.length<1){
			...
			selectCheckbox.each(function(i){
			 ...
			 art.dialog({
			    ...
			    ok: function () {
			    	$.ajax({
						url:"<%=basePath%>admin/supplier_disableByIds.Q",
						data:{"ids":ids},
						dataType:'text',
						success:function(data){
							if("ok"==data){
		...
		if(selectCheckbox.length<1){
			...
			art.dialog({
			    ...
			    ok: function () {
			    	$.ajax({
						url:"<%=basePath%>admin/supplier_enableByIds.Q",
						data:{"ids":ids},
						dataType:'text',
						success:function(data){
							if("ok"==data){
								var url = "<%=basePath%>admin/supplier_manager.Q";
								window.location.href=url;
							}else{
								art.dialog({
									time: 2,
	...
	function importExcel(){
		var url = "<%=basePath%>admin/supplier_toImport.Q";
		art.dialog.open(url, {title: '供应商导入', lock: true, drag:true, width: getClientWidth()*0.8, height: getClientHeight()*0.85});
	}
	
	function exportExcel(){
		var param = $("#queryForm").serialize();
		var url =  "<%=basePath%>admin/supplier_exportSupplierData.Q";
		if(param){
			url+="?"+param;
		}
		window.location.href = url;
	...
	function deleteByQuery(){
		...
		art.dialog({
			    ...
			    ok: function () {
			    	$.ajax({
						url:"<%=basePath%>admin/supplier_deleteByQuery.Q",
						data:queryParam,
						dataType:'text',
						success:function(data){
							if("ok"==data){
...
<div class="container-fluid">
      <div class="widget-box">
        <div class="widget-content">
          <form action="admin/supplier_manager.Q" method="post" id="queryForm">
            <div class="controls">
	            <label class="span1" for="inputSuccess" style="margin-top:5px;width:56px;">名称</label>
	            <input name="vcFactoryName" id="vcFactoryName" type="text" class="span2"  value="${ vcFactoryName}">
	            <label class="span1" for="inputSuccess" style="margin-top:5px;width:56px;">代码</label>
	         ...
	         <div class="clear"></div>
	         <div class="controls">
	            <label class="span1" for="inputSuccess" style="margin-top:5px;width:56px;">产地</label>
	            <input name="vcPlaceProduce" id="vcPlaceProduce" type="text" class="span2"  value="${ vcPlaceProduce}">
	            <label class="span1" for="inputSuccess" style="margin-top:5px;width:56px;">联系人</label>
	            <input name="vcLinkMan" id="vcLinkMan" type="text" class="span2"  value="${vcLinkMan }">
	              <label class="span1" for="inputSuccess" style="margin-top:5px;width:56px;">备注</label>
	            <input name="vcRemarks" id="vcRemarks" type="text" class="span2"  value="${ vcRemarks}">
        ...
        <div class="widget-title"> <span class="icon"> <i class="icon-th"></i> </span>
            <h5>供应商列表</h5>
             <w:permission permissionId="<%=PermissionId.BASIC_SUPPLIER_MGT_ADD%>">
            <button class="label label-info btn btn-primary btn-mini" onclick="addOrUpdate();">新增</button>
         	 <button class="label label-info btn btn-primary btn-mini" onclick="importExcel();">导入</button>
         	  ...
         	  <w:permission permissionId="<%=PermissionId.BASIC_SUPPLIER_MGT_DELETE%>">
            ...
            <button class="label label-info btn btn-primary btn-mini" onclick="enableByIds();">启用</button>
             ...
             <w:permission permissionId="<%=PermissionId.BASIC_SUPPLIER_MGT_DOWNLOAD%>">
