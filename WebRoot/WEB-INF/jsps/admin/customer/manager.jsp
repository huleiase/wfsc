...
<script type="text/javascript">
	...
	function addOrUpdate(id){
		var url = "<%=basePath%>admin/customer_input.Q";
		if(id){
			url+="?id="+id;
		}
	...
	function deleteByIds(){
		...
		if(selectCheckbox.length<1){
			...
			art.dialog({
			    ...
			    ok: function () {
			    	$.ajax({
						url:"<%=basePath%>admin/customer_deleteByIds.Q",
						data:{"ids":ids},
						dataType:'text',
	...
	function disableByIds(){
		...
		if(selectCheckbox.length<1){
			...
			selectCheckbox.each(function(i){
			 ...
			 art.dialog({
			    ...
			    ok: function () {
			    	$.ajax({
						url:"<%=basePath%>admin/customer_disableByIds.Q",
						data:{"ids":ids},
						dataType:'text',
						success:function(data){
							if("ok"==data){
		...
		if(selectCheckbox.length<1){
			...
			art.dialog({
			    ...
			    ok: function () {
			    	$.ajax({
						url:"<%=basePath%>admin/customer_enableByIds.Q",
						data:{"ids":ids},
						dataType:'text',
						success:function(data){
							if("ok"==data){
								var url = "<%=basePath%>admin/customer_manager.Q";
								window.location.href=url;
							}else{
								art.dialog({
									time: 2,
...
<div class="container-fluid">
      <div class="widget-box">
        <div class="widget-content">
          <form action="admin/customer_manager.Q" method="post" id="queryForm">
            <div class="controls">
        ...
        <div class="widget-title"> <span class="icon"> <i class="icon-th"></i> </span>
            <h5>供应商列表</h5>
            <w:permission permissionId="<%=PermissionId.BASIC_CUSTOMER_MGT_ADD%>">
            <button class="label label-info btn btn-primary btn-mini" onclick="addOrUpdate();">新增</button>
         	 ...
         	 <w:permission permissionId="<%=PermissionId.BASIC_CUSTOMER_MGT_DELETE%>">
            ...
            <w:permission permissionId="<%=PermissionId.BASIC_CUSTOMER_MGT_DOWNLOAD%>">
