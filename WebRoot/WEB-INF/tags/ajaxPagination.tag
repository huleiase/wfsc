...
<%@ attribute name="scripts" type="java.lang.String" required="false"%>
	...
	<h2> 
		<span onmouseout="this.className='h2_span'" onmouseover="this.className='h2_span_on'" class="h2_span">
			<i>
				...
				<%} else { %>
				<a  href="javascript:void(0);" onclick="javascript: ajaxPage(1);return false;">
					<img class="feng_img" src="<%=request.getContextPath()%>/images/vf2.gif" height="10" width="5">
					<img class="feng_img" src="<%=request.getContextPath()%>/images/vf2.gif" height="10" width="5">
				</a>
				<%} %>
		...
		<!--<s:iterator value="#request.li" var="pi">
			...
			<s:else>
			<span onmouseout="this.className='h2_span'" onmouseover="this.className='h2_span_on'" class="h2_span" onclick="ajaxPage('<s:property value="pi"/>');return false;"><i><s:property value="pi"/></i></span>
			</s:else>
		</s:iterator>-->
		<c:forEach items="${li}" var="i">
			<c:choose>
				 ...
				 <c:otherwise>
				  	<span onmouseout="this.className='h2_span'" onmouseover="this.className='h2_span_on'" class="h2_span" onclick="ajaxPage(${i});return false;"><i>${i }</i></span> 
				 </c:otherwise>
			</c:choose>
		</c:forEach>
		<span onmouseout="this.className='h2_span'" onmouseover="this.className='h2_span_on'" class="h2_span">
			<i>
				...
				<% }else { %>
				<a href="javascript:void(0);" onclick="javascript: ajaxPage(${totalPage });return false;">
					<img class="feng_img" src="<%=request.getContextPath()%>/images/vf3.gif" height="10" width="5">
					<img class="feng_img" src="<%=request.getContextPath()%>/images/vf3.gif" height="10" width="5">
				</a>
				<%}%>				
		...
		<span style="height:10px;line-height:12px;">
			...
			<input style="border: medium none ; margin: 0pt 0pt 0pt 4px; background: transparent url(<%=request.getContextPath()%>/images/2_r2_c2.jpg) repeat scroll 0% 0%; -moz-background-clip: -moz-initial; -moz-background-origin: -moz-initial; -moz-background-inline-policy: -moz-initial; width: 40px; height: 20px; cursor: pointer; color: rgb(68, 68, 68);" value="转到" onclick="ajaxPage(document.getElementById('destPage').value);" type="button"> 
			每页&nbsp;
			<select id="pageSize" name="page.pageSize" onchange="javascript: ajaxPage(1)" style="width: 55px;height:24px;">
