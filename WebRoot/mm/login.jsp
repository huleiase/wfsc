...
<head>
        <title>广州诚光进销存后台管理系统</title><meta charset="UTF-8" />
    ...
    <body>
        <div id="loginbox">      
            <s:form action="admin_login.Q" namespace="/admin" method="POST" onsubmit="return checkLogin();" id="lgForm">
				 <div class="control-group normal_text"> <h3><img src="<%=request.getContextPath()%>/mm/img/logo.png" alt="Logo" /></h3></div>
                ...
                <div class="control-group">
                    <div class="controls">
                        <div class="main_input_box">
                            <span class="add-on bg_ls"><i class="icon-th"></i></span><input name="verifyCode" id="verifyCode" type="text" placeholder="请输入验证码" autocomplete="off"/>
                           </br> <span style="color: #F00;display:none" id="verifyCodeSpan">请输入验证码</span>
                            </br> <img src="/wfsc/getCodeImage?flag=admin" style="width: 100px;height: 40px; cursor: pointer;" alt="点击切换验证码" title="点击切换验证码" onclick="changeCode(this);">
                        </div>
                    </div>
                </div>
                <div class="form-actions">
                    <span class="pull-left" style="display: none"><a href="#" class="flip-link btn btn-info" id="to-recover">忘记密码?</a></span>
                    <span class="pull-right"><a href="javascript:void(0);" class="btn btn-success" onclick="doLogin()"/> 登录</a></span>
                </div>
			</s:form>            
        ...
        <script type="text/javascript">
        	function changeCode(obj){
				obj.src = '/wfsc/getCodeImage?s=' + Math.random();
        	}
